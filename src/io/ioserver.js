...
class IOServer {
  constructor(
    ...
    ({ proxyTrustFn: this.proxyTrustFn } = options);
  ...
  ipProxyMiddleware(socket, next) {
    ...
    try {
      ...
      if (socket.handshake.headers.hasOwnProperty("cf-connecting-ip")) {
        socket.context.ipAddress = socket.handshake.headers["cf-connecting-ip"];
      } else {
  ...
  ipThrottleMiddleware(socket, next) {
    ...
    if (bucket.throttle()) {
      ...
      LOGGER.info(
        "Rejecting %s - exceeded connection rate limit",
  ...
  checkIPLimit(socket) {
    ...
    if (count >= Config.get("io.ip-connection-limit")) {
      ...
      LOGGER.info("Rejecting %s - exceeded connection count limit", ip);
      socket.emit("kick", {
  ...
  ipSessionCookieMiddleware(socket, next) {
    ...
    const sessionMatch = verifyIPSessionCookie(
      socket.context.ipAddress,
      cookie
    );
    if (sessionMatch) {
  ...
  authUserMiddleware(socket, next) {
      ...
      promises.push(
        verifySession(auth)
          ...
          .catch((_error) => {
            ...
            LOGGER.warn(
              "Unable to verify session for %s - ignoring auth",
              socket.context.ipAddress
            );
          })
      );
    ...
    promises.push(
      getAliases(socket.context.ipAddress)
        .then((aliases) => {
          socket.context.aliases = aliases;
        })
        .catch((_error) => {
          LOGGER.warn(
            "Unable to load aliases for %s",
            socket.context.ipAddress
          );
        })
    );
  ...
  try {
    ...
    sock.conn.on("upgrade", () => {
      try {
        ...
        LOGGER.error(
          "Error emitting transport upgrade metrics for socket (ip=%s): %s",
          sock.context.ipAddress,
          error.stack
        );
      }
    });
    ...
    sock.once("disconnect", () => {
      try {
        ...
        LOGGER.error(
          "Error emitting disconnect metrics for socket (ip=%s): %s",
          sock.context.ipAddress,
          error.stack
        );
      }
    ...
    sock.once("reportReconnect", () => {
      try {
        ...
        LOGGER.error(
          "Error emitting reconnect metrics for socket (ip=%s): %s",
          sock.context.ipAddress,
          error.stack
        );
      }
    ...
    LOGGER.error(
      "Error emitting metrics for socket (ip=%s): %s",
      sock.context.ipAddress,
      error.stack
    );
  }
...
function getCorsAllowCallback() {
  let origins = Array.prototype.slice.call(
    Config.get("io.cors.allowed-origins")
  );
  ...
  return function corsOriginAllowed(origin, callback) {
    ...
    if (origins.includes(origin)) {
      ...
      LOGGER.warn(
        'Rejecting origin "%s"; allowed origins are %j',
        origin,
        origins
      );
